...
export function validateResponse(response,request,m,{now=Date.now(),mode='live'}={}){
 ...
 for(const q of response.quotes){
  ...
  if(p.family==='insurance'){
   ...
   for(const c of requested)need(v.coverages.some(x=>isDeepStrictEqual(x,c)),'Coverage differs from requested terms; counteroffers require a reviewed extension');
  }else if(p.family==='credit'){
   ...
   const v=q.retirement;need(v.initialCapital===request.input.initialCapital&&isDeepStrictEqual(v.contribution,request.input.contribution),'Retirement contribution mismatch');
